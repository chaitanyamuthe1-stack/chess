...
def do_move(fr, fc, tr, tc, promo=None, silent=False):
    piece = st.session_state.board[fr][fc]
    is_promotion = ptype(piece) == 'P' and (tr == 0 or tr == 7)
    
    if is_promotion and not promo:
        st.session_state.pending_promo = (fr, fc, tr, tc)
        return
    
    new_board, captured, new_castling, new_en_passant = apply_move(
        st.session_state.board,
        fr, fc, tr, tc,
        promo,
        st.session_state.castling,
        st.session_state.en_passant
    )
    ...
    st.session_state.history.append({
        ...
        'castling': st.session_state.castling.copy(),
        'en_passant': st.session_state.en_passant
    })
    
    st.session_state.board = new_board
    st.session_state.castling = new_castling
    st.session_state.en_passant = new_en_passant
    ...
    st.session_state.move_history.append({
        ...
        'promo': promo if promo and promo != 'Q' else ''
    })
    ...
    if not silent:
        ...
        if promo and promo != 'Q':
            msg += f"  ={promo}"
        if abs(tc - fc) == 2 and ptype(piece) == 'K':
            msg += '  (castling)'
        add_log(cls, msg)
    ...
    legal = get_legal_moves(st.session_state.board, st.session_state.turn, st.session_state.castling, st.session_state.en_passant)
    ...
    if st.session_state.selected:
        ...
        if (sr, sc) == (r, c):
            ...
            if st.session_state.board[r][c] and color(st.session_state.board[r][c]) == 'w':
                ...
                legal = [m for m in get_legal_moves(
                    st.session_state.board,
                    'w',
                    st.session_state.castling,
                    st.session_state.en_passant
...
with right:
    ...
    if st.session_state.pending_promo:
        fr, fc, tr, tc = st.session_state.pending_promo
        st.markdown("### Promotion")
        promo_cols = st.columns(4)
        for idx, promo_piece in enumerate(['Q', 'R', 'B', 'N']):
            with promo_cols[idx]:
                if st.button(f"Promote to {promo_piece}", key=f"promo_{promo_piece}"):
                    do_move(fr, fc, tr, tc, promo=promo_piece)
                    st.rerun()
        st.markdown("---")
        ...
        if st.button("↩ Undo", use_container_width=True):
            if len(st.session_state.history) >= 2:
                ...
                if st.session_state.history:
                    ...
                    st.session_state.castling = prev.get('castling', st.session_state.castling.copy())
                    st.session_state.en_passant = prev.get('en_passant', None)
                else:
                    init_game()
...
if st.session_state.ai_vs_ai and not st.session_state.game_over:
    ...
    if move:
        do_move(move['fr'], move['fc'], move['tr'], move['tc'], promo=move.get('promo'), silent=True)
        st.rerun()
    ...
    st.session_state.nodes = nodes
    if move:
        do_move(move['fr'], move['fc'], move['tr'], move['tc'], promo=move.get('promo'))
        st.rerun()
